    ...
    function setupLogging(config) {
      if (self.config.debugLogging) {
        ...
        if (self.config.debugLogging === 'realtime-dangerous') {
          ...
          // Recursively redact any references to mail headers and any detail
          // properties starting with an underscore.
          function censorObject(x) {
            for (var key in x) {
              var value = x[key];
              if (key[0] === '_' || (value && (value.suid || value.srvid))) {
                x[key] = '(redacted)';
          ...
          logic.setEventCensor((event) => {
            censorObject(event.details);
          });
        }
        else {
          console.warn('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');
...
MailUniverse.prototype = {
  ...
  createLogBacklogRep: function() {
    return {
      type: 'logic',
      events: this._logBacklog.map((e) => e.toJSON())
    };
  },
